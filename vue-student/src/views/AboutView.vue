...
const fetchCategories = async () => {
  try {
    const response = await request.get<any, any>('/system/category/withBookCount');
    const resData = response.data || response;
    if (resData && typeof resData === 'object' && 'code' in resData && resData.code === 200) {
...
const fetchMaterials = async () => {
  try {
    const response = await request.get<any, any>('/system/goods/list', {
      params: {
    ...
    const resData = response.data || response;
    if (resData && typeof resData === 'object' && 'code' in resData && resData.code === 200 && Array.isArray(resData.rows)) {
      materials.value = resData.rows;
      total.value = resData.total || 0;
    } else if (resData && typeof resData === 'object' && 'msg' in resData) {
